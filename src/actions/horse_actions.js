...
export const horseCreate = ({horsename,birth,breed, gender,familly,withers,girthFloor,heartGirth,
    ...
    .push({horsename,birth,breed, gender,familly,withers,girthFloor,heartGirth,
        length,shoulderGirth,trained,isNervous}).then((e)=>{

        dispatch({ type: 'HORSE_CREATED',
                    id:e.key});
    });
    };
};
export const horseCreatedFetch = ({id}) =>{
    ...
    return(dispatch) =>{
        ...
        .on('value', snapshot=>
        {
            dispatch({ type: 'HORSE_CREATED_FETCH',
...
export const updateHorse = ({horsename,birth,breed, gender,familly,withers,girthFloor,heartGirth,
    ...
    return async (dispatch) =>{
        ...
        .ref(`/users/${currentUser.uid}/horses/${id}`).update({horsename,birth,breed
...
export const horsesFetch = () =>{
    ...
    return(dispatch) =>{
        ...
        .ref(`/users/${currentUser.uid}/horses`)
        .on('value', snapshot=>{
            dispatch({ type: 'HORSES_FETCH_SUCCESS',
        payload: snapshot.val() });
        });
